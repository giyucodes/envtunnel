...
function Countdown({ expiresAt }: { expiresAt: number }) {
  ...
  return (
    <span
      style={{
        color: remaining < 60 ? "var(--danger)" : "var(--warning)",
        fontSize: 12,
        fontWeight: 600,
      }}
    >
  ...
  return (
    <div style={{ border: "1px solid var(--border)", marginBottom: 24 }}>
      ...
      >
        ...
        {open ? (
          <ChevronUp size={13} color="var(--muted2)" />
        ) : (
          <ChevronDown size={13} color="var(--muted2)" />
        )}
      </button>
      ...
      {open && (
        ...
        >
          ...
          >
            Use your own Upstash Redis for full privacy — your env vars never
            touch the server's storage. Get free creds at{" "}
            <a
...
export default function Dashboard() {
  ...
  const handlePull = async () => {
    ...
    try {
      ...
      await fetch(`/api/tunnel/consume/${pullToken.trim()}${qs}`, {
        method: "DELETE",
      });
      ...
      if (err.name === "OperationError" || err.message?.includes("decrypt")) {
        setPullError(
          "Wrong passphrase — decryption failed. Token is still alive, try again.",
        );
      } else {
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              {pushResult && (
                ...
                >
                  ...
                  >
                    <Clock size={12} />
                    expires in <Countdown expiresAt={pushResult.expiresAt} /> —
                    one-time use
                  </div>
                  <div
                    style={{
                  ...
                  >
                    ⚠ Share the passphrase via a separate channel (Signal, phone
                    call, etc.)
                  </div>
                </div>
              )}
          ...
          {tab === "pull" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              <button
                ...
                onClick={handlePull}
                disabled={
                  pulling || !pullToken.trim() || !pullPassphrase.trim()
                }
                style={{ alignSelf: "flex-start" }}
              >
                <Download size={14} />
              ...
              {pullResult && (
                ...
                >
                  ...
                  >
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      ...
                      <button
                        ...
                        onClick={() => {
                          const blob = new Blob([pullResult], {
                            type: "text/plain",
                          });
                  ...
                  >
                    ✓ Token consumed after successful decryption. Cannot be
                    pulled again.
                  </div>
                </div>
              )}
          ...
          >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              <div
                ...
                style={{ padding: "8px 12px", fontSize: 11 }}
              >
                <span style={{ color: "var(--accent)" }}>$</span> envtunnel pull{" "}
                <span style={{ color: "var(--muted2)" }}>[token]</span>
              </div>
            </div>
          </div>
        </div>
